...
Canonweave keeps the thread between what was decided and what was built:
...
## 0. Get canonweave (one minute)

Until the npm publish lands (WS5), canonweave runs straight from a checkout —
there is deliberately no install step:

```bash
git clone https://github.com/canonweave/canonweave ~/canonweave   # or use an existing checkout
canonweave() { node ~/canonweave/packages/cli/bin/canonweave.mjs "$@"; }
canonweave selftest   # optional: proves your checkout works (~5s, no network)
```
...
```bash
canonweave init --template generic-software --dir my-product
cd my-product
```
...
```
canonweave.yml            # repo config: roots, ontology, graph, default gate
...
```bash
canonweave check   # human report: suspects, coverage gaps, per-profile verdicts
canonweave gate    # exit 0 — the ready-to-build profile passes
```

(`canonweave` here = `node <repo>/packages/cli/bin/canonweave.mjs`; alias it.)
...
canonweave gate; echo $?   # 1 — the gate is red until you reconcile
```

## 4. Reconcile (the ripple is the point)

```bash
canonweave reconcile requirements          # drafts a corrected artifact -> .canonweave/proposals/
# review the proposal, then:
canonweave reconcile requirements --apply  # applies the draft, clears the suspect link
canonweave check                           # requirements is fresh — but now ITS dependents went suspect
```
...
Repeat `canonweave reconcile <id> --apply` for whatever `check` lists as
suspect (top-down), then:

```bash
canonweave gate; echo $?                   # 0 — green again, the whole chain re-reviewed
```

If the upstream change does not actually invalidate a downstream (no rewrite
needed), mark the link reviewed instead: `canonweave clear <id> <ingredient>`.
...
```bash
cp ~/canonweave/templates/github/canonweave-gate.yml .github/workflows/
```
...
canonweave checkout — vendor the repo (or a submodule) and use
`uses: ./<path-to>/packages/action`; the published `canonweave/canonweave` form is
noted in the template for later. Any non-GitHub CI can gate with the CLI
directly:

```yaml
- run: node <canonweave>/packages/cli/bin/canonweave.mjs gate
```
